...
export default {
  ...
  data() {
    return {
      baseUrl: process.env.VUE_APP_BASE_API, // 上传的路径
      // 设置请求头部
      headers: {
      ...
      const newFileList = fileList.filter((item) => {
        if (item.response) {
          if (item.response.code === 200) {
            if (
              typeof item.response.data.video_url === 'string' ||
              typeof item.response.data === 'string'
            ) {
              item.url = item.response.data.video_url || item.response.data
            } else {
              item.url = ''
            }
            item.resData = item.response.data
            // delete item.response
            return item
          } else {
            this.$message.error(item.response.message || '上传失败')
          }
