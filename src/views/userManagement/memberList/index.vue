<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      <el-input
        v-model="listQuery.nickname" clearable class="filter-item" style="width: 200px;"
        placeholder="请输入昵称"
        @keyup.enter.native="getList"
      />
      <el-input
        v-model="listQuery.mobile" clearable class="filter-item" style="width: 200px;"
        placeholder="请输入电话"
        @keyup.enter.native="getList"
      />
      <el-select v-model="listQuery.userLevel" clearable class="filter-item" style="width: 200px;" placeholder="选择层级">
      ...
      />
      <el-button
        v-permission="[ `GET /admin${api.userList}` ]" size="mini" type="primary" icon="el-icon-search"
        class="filter-item" @click="handleSearch"
      >
        查找
      </el-button>
    </div>
    ...
    <TableTools
      ...
      @refresh="getList" @download="toolsMixin_exportMethod($refs.vxeTable, '会员管理')"
    >
      <!-- 超管专属 -->
      ...
      <template v-if="isShopRole">
        <el-button
          v-permission="[ `POST /admin${api.orderSVsAdd}` ]" :disabled="orderSVsAddBtnDisabled"
          icon="el-icon-lock" type="success" size="mini" @click="handleAssign(true)"
        >
          指派业务员
        </el-button>
        <el-button
          v-permission="[ `POST /admin${api.orderSVsDeleted}` ]" :disabled="orderSVsDeletedBtnDisabled"
          icon="el-icon-unlock" type="danger" size="mini" @click="handleAssign(false)"
        >
          解除业务员
        </el-button>
      </template>
    ...
    >
      ...
      <template #operate="{ row }">
        <el-button v-permission="[ `POST /admin${api.userUpdate}` ]" size="mini" @click="handleEdit(row)">
          编辑
        </el-button>
        <!-- 超管专属 -->
        <template v-if="isAdminRole">
          ...
          <el-button
            v-permission="[ `POST /admin${api.partnerApplySaveAndSignin}` ]" :disabled="row.userLevel !== 5"
            type="warning" @click="handlePartnerApply(row, 6)"
          >
            会员升级
          </el-button>
          <el-button
            v-permission="[ `POST /admin${api.partnerApplySaveAndSignin}` ]" :disabled="row.userLevel !== 6"
            type="warning" @click="handlePartnerApply(row, 7)"
          >
            合伙人升级
          </el-button>
        </template>
            ...
            <table cellspacing="0" style="width: 100%;">
              ...
              <tbody>
                <tr v-for="item in RoleCountList" :key="item.userLevel">
                  <td>
                    <div class="cell">{{ item.userLevelDesc }}</div>
                  </td>
                  <td>
