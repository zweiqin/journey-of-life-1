<template>
  <div class="app-container">
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <el-button
            type="primary"
            @click="storeHandleUpdate(scope.row)"
          >门店提交</el-button>
          <el-button
            type="danger"
            @click="marketHandleUpdate(scope.row)"
          >营销提交</el-button>
        </template>
      </el-table-column>
    </el-table>
      ...
      >
        ...
        <el-form-item label="账户头像" prop="avatar">
          <el-upload
            ...
            <img v-if="storeDataForm.avatar" :src="storeDataForm.avatar" class="avatar">
        ...
        >
          <el-select
            v-model="storeDataForm.brandgenre"
            placeholder="请选择所属类型"
            autocomplete="off"
          >
            <el-option
        ...
        >
          <el-time-select
            v-model="storeDataForm.startTime"
            :picker-options="{
        ...
        >
          ...
          >
            <img
              v-if="storeDataForm.picUrl"
              :src="storeDataForm.picUrl"
              class="avatar"
            >
            <i
              v-else
        ...
        >
          ...
          >
            <img
              v-if="storeDataForm.licenseUrl"
              :src="storeDataForm.licenseUrl"
              class="avatar"
            >
            <i
              v-else
        ...
        <el-form-item
          label="法人身份证正面"
          prop="brandIdcardProsUrl"
        >
          <el-upload
            :headers="headers"
            :action="uploadPath"
            :show-file-list="false"
            :on-success="uploadBrandIdcardProsUrl"
            class="avatar-uploader"
            accept=".jpg,.jpeg,.png,.gif"
          >
            <img
              v-if="storeDataForm.brandIdcardProsUrl"
              :src="storeDataForm.brandIdcardProsUrl"
              class="avatar"
            >
            <i
              v-else
        ...
        <el-form-item
          label="法人身份证反面"
          prop="brandIdcardConsUrl"
        >
          <el-upload
            :headers="headers"
            :action="uploadPath"
            :show-file-list="false"
            :on-success="uploadBrandIdcardConsUrl"
            class="avatar-uploader"
            accept=".jpg,.jpeg,.png,.gif"
          >
            <img
              v-if="storeDataForm.brandIdcardConsUrl"
              :src="storeDataForm.brandIdcardConsUrl"
              class="avatar"
            >
            <i
              v-else
        ...
        >
          <el-date-picker
            v-model="storeDataForm.birthday"
            type="date"
            placeholder="选择日期"
    ...
    <!-- 营销提交对话框 -->
    <el-dialog
      :visible.sync="marketDialogFormVisible"
      title="营销端提交申请"
    >
      <el-form
        ref="marketDataForm"
        :rules="marketRules"
        :model="marketDataForm"
        status-icon
        label-position="left"
        label-width="140px"
        style="width: 400px; margin-left:50px;"
      >
        ...
        >
          <el-date-picker
            v-model="marketDataForm.birthday"
            type="date"
            placeholder="选择日期"
            value-format="yyyy-MM-dd"/>
        </el-form-item>
        ...
        >
          ...
          >
            <img
              v-if="marketDataForm.idcardProsUrl"
              :src="marketDataForm.idcardProsUrl"
              class="avatar"
            >
            <i
              v-else
        ...
        >
          ...
          >
            <img
              v-if="marketDataForm.idcardConsUrl"
              :src="marketDataForm.idcardConsUrl"
              class="avatar"
            >
            <i
              v-else
      ...
      >
        ...
        <el-button
          type="primary"
          @click="marketSaveData"
        >保存</el-button>
        <el-button
          type="danger"
          @click="marketUpdateData"
        >提交</el-button>
      </div>
    </el-dialog>
...
export default {
  ...
  data () {
    return {
      ...
      storeRules: {
        ...
        brandIdcardProsUrl: [{ required: true, message: '法人身份证正面不能为空', trigger: 'blur' }],
        brandIdcardConsUrl: [{ required: true, message: '法人身份证反面不能为空', trigger: 'blur' }]
      },
      marketRules: {
  ...
  methods: {
    ...
    getStoreTypeList () {
      listStoreType()
        .then((response) => {
          this.listStoreType = response.data.items
          this.storeDataForm.brandgenre = response.data.items[0].id
        })
        .catch((error) => {
          this.$message({
            showClose: true,
    ...
    uploadAvatarUrl: function (response) {
    ...
    uploadBrandIdcardProsUrl: function (response) {
      this.storeDataForm.brandIdcardProsUrl = response.data.url
    },
    uploadBrandIdcardConsUrl: function (response) {
