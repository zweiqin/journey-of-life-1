<template>
  <div class="app-container">
    ...
    <!-- 超级管理员端查询结果 -->
    <el-table v-loading="listLoading" v-show="isAdmin" :data="list" size="small" element-loading-text="正在查询中..." border fit highlight-current-row @expand-change="expand" >
        ...
        <template slot-scope="scope">
          ...
          >
            ...
            <el-table-column align="center" label="操作" width="200" class-name="small-padding fixed-width">
              <template slot-scope="scope">
                <el-button
                  v-permission="['POST /admin/buAccount/topUp']"
                  type="primary"
                  size="mini"
          ...
          <pagination
            v-show="totalId[scope.row.id]>0"
            :total="Number(totalId[scope.row.id])"
            :page.sync="listIdQuery.page"
            :limit.sync="listIdQuery.limit"
    ...
    <pagination v-show="isAdmin && total>0" :total="total" :page.sync="listQuery.page" :limit.sync="listQuery.limit" @pagination="getList" />

    <!-- 门店端查询结果 -->
    <el-table v-loading="listIdLoading" v-show="!isAdmin" :data="listId[1]" size="small" element-loading-text="正在查询中..." border fit highlight-current-row >
      <el-table-column align="center" width="100px" label="编号" prop="id" sortable/>
    ...
    <pagination
      v-show="!isAdmin && totalId[1]>0"
      :total="Number(totalId[1])"
      :page.sync="listIdQuery.page"
      :limit.sync="listIdQuery.limit"
      @pagination="getListOfId(1)"
    />

    <!-- 充值 -->
    <el-dialog :visible.sync="rechargeVisible" title="充值中心" width="40%">
      <el-form ref="rechargeForm" :model="rechargeForm" status-icon label-position="left" label-width="100px" style="width: 350px; margin-left:50px;">
        <el-form-item
          label="会员id"
          prop="userId"
        >
