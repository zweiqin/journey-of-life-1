...
<script>
import { fetchList, approveAgency, detailApprove, levelList } from '@/api/business/user'
...
export default {
  ...
  data () {
    return {
      ...
      genderDic: ['未知', '男', '女'],
      levelDic: ['普通用户', '门店', '营销策划师', '', 'VIP会员', '会员'],
      statusDic: ['可用', '禁用', '注销', '其他'],
  ...
  methods: {
    ...
    getLevelList () {
      this.listLoading = true
      levelList(this.listQuery).then(response => {
        this.list = response.data.items
        this.total = response.data.total
        this.listLoading = false
      }).catch(() => {
