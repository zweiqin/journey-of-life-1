<template>
  <div class="base">
    ...
    <div :style="{padding:10* $root.dw+'px'}" class="fristdiv">
      <div v-for="value,index in list" :key="list[index].img_id" :style="{padding:20* $root.dw+'px'}" class="divitem">
        <el-card :body-style="{ padding: '0px',height:'100%',width:'100%' ,cursor:'pointer'}">
          <img :src="list[index].img_url" style="width:100%;height:100%" @click="showPicture(list[index].img_url,list[index].img_title)">
          <div class="TXBJW">
    ...
    <el-dialog :visible.sync="detailDialogVisible">
      <img :src="lookingPicture" width="100%">
      <div :style="{width:'100%',fontSize:20*$root.dw+'px'}" class="TXBJW">{{ lookingPictureTitle }}</div>
    </el-dialog>
  </div>
</template>
