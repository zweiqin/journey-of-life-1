    ...
    >
      ...
      >
        <template slot-scope="scope">
          ...
          <el-tag v-if="scope.row.status === 3" type="danger">代金券未发放</el-tag>
          <el-tag v-if="scope.row.status === 4" type="success">代金券已发放</el-tag>
        </template>
      </el-table-column>
...
import { splicingDate } from '@/assets/js/common'
      ...
      listGet(this.listQuery)
        .then(response => {
          ...
          for (const i in this.list) {
            this.list[i].payTime = splicingDate(this.list[i].payTime)
          }
