  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column type="expand">
        <template slot-scope="scope">
          <!-- 二级列表 -->
          <el-input
            v-model="listQueryId.name"
            clearable
            size="mini"
            class="filter-item"
            style="width: 200px;"
            placeholder="请输入账号或名称"
            @keyup.enter.native="rowHandleFilter(scope.row)"
          />
          <el-button
            v-permission="['']"
            size="mini"
          ...
          <el-table
            ...
            ref="salesman"
            :data="listId[scope.row.salesmanid + 'a']"
            size="small"
            element-loading-text="正在查询中。。。"
            border
            fit
          ...
          >
            ...
            >
              <template slot-scope="scope">
                <el-button
                  type="danger"
                  @click="handleIdDelete(scope.row)"
                >解绑</el-button>
              </template>
            </el-table-column>
          </el-table>
          ...
          <pagination
            v-show="IdTotal[scope.row.salesmanid + 'a']>0"
            :total="Number(IdTotal[scope.row.salesmanid + 'a'])"
    ...
    >
      ...
      >
        ...
        >
          <el-input
            v-model="dataFormListQuery.name"
            clearable
            size="mini"
            class="filter-item"
            style="width: 200px;"
            placeholder="请输入账号或名称"
            @keyup.enter.native="dialogHandleFilter()"
          />
          <el-button
            v-permission="['']"
            size="mini"
          ...
          <pagination
            ...
            :page.sync="dataFormListQuery.page"
            :limit.sync="dataFormListQuery.limit"
            @pagination="GetUnbindingStore"
          />
        </el-form-item>
      </el-form>
      <div
      ...
      >
        ...
        >
          <el-table
            v-loading="listIdLoading"
            ref="salesman"
            :data="listId[bindingDataForm.administratorId + 'a']"
            size="small"
            element-loading-text="正在查询中。。。"
            border
            fit
  ...
  methods: {
    ...
    getListOfId (salesmanid) {
      ...
      idGet(this.listQueryId).then(response => {
        // 设置列表数据
        this.$set(this.listId, salesmanid + 'a', response.data.items)
        // 设置列表页
    ...
    GetUnbindingStore () {
      ...
      userGet(this.dataFormListQuery).then(response => {
        this.$set(this.addDataFormListTotal, this.bindingDataForm.administratorId + 'a', response.data.total)
