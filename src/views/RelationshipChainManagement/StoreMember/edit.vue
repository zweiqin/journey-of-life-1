...
export default {
  ...
  methods: {
    init: function () {
      ...
      //   this.editData.salesmanId = response.data.salesmanId
      // })
    ...
    getUserIdOptions () {
      idGet().then(response => {
        ...
        this.userIdOptions = response.data.items
      }).catch(response => {
    ...
    getHeadUid (id) {
      ...
      adminGet(this.listQueryId).then(response => {
        this.userIdOptions = response.data.items
        this.editData.old_user_id = response.data.items.map((item) => {
          return item.id
        })
        this.editData.new_user_id = response.data.items.map((item) => {
          return item.id
        })
