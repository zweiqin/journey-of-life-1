  ...
  methods: {
    init () {
      ...
      if (this.$route.query.lastRouter === 'brandListShow') {
        ...
        goodsCatAndBrand(this.goods.brandId).then(response => {
    ...
    handlePublish () {
      const finalGoods = {
        goods: {
          ...this.formData,
          gallery: Array.isArray(this.formData.gallery) ? this.formData.gallery.map(v => v.resData) : [],
          is_deliveryDay: this.formData.is_deliveryDay ? '0' : this.formData.deliveryDay,
          limitDays: this.formData.timeType === 0 ? this.formData.limitDays : '',
          endTime: this.formData.timeType === 1 ? this.formData.endTime : '',
        },
