  ...
  <div class="app-container">
    ...
    <div class="filter-container">
      <!-- <el-input
        ...
        <br /> -->
      <el-button
        v-permission="[ `POST /admin${api.categoryCreate}` ]" size="mini" type="primary" icon="el-icon-plus"
    ...
    <div v-tableHeight>
      ...
      <el-table
        ...
        :data="list"
        v-bind="$tableCommonOptions" row-key="id" border lazy
        :load="load" :default-expand-all="false"
      ...
      >
        ...
        <el-table-column align="center" label="操作" width="120" fixed="right" class-name="small-padding fixed-width">
          <template slot-scope="{ row }">
            <el-button v-permission="[ `POST /admin${api.categoryUpdate}` ]" size="mini" @click="handleUpdate(row)">
              编辑
            </el-button>
            <el-button
              v-permission="[ `POST /admin${api.categoryDelete}` ]" type="danger" size="mini"
  ...
  methods: {
    ...
    async load(tree, treeNode, resolve) {
      ...
      try {
        ...
        if (res.data.total === 0) this.$elMessage(`没有更多数据了！`, 'warning')
      } finally {
        this.listLoading = false
      }
    },
